...
export class EthicsValidator {
  private ethicsPath: string;

  constructor(repoPath: string) {
    this.ethicsPath = path.join(repoPath, 'ethics');
  }

  async getCurrentEthics(): Promise<Record<string, EthicsFramework>> {
    const ethics: Record<string, EthicsFramework> = {};
    ...
    try {
      ...
      if (latestCoreVersion) {
        ...
        for (const file of coreFiles.filter(f => f.endsWith('.yml') || f.endsWith('.yaml'))) {
          ...
          const ethicsData = yaml.load(content) as EthicsFramework;
  ...
  async validateProposal(proposal: GovernanceProposal): Promise<EthicsCompatibilityResult> {
    ...
    const relevantEthics = Object.entries(currentEthics).filter(([key, _]) => 
      key.includes('core-') || key.includes(`${proposal.domain}-`)
    );
  ...
  private checkPrincipleCompliance(proposal: GovernanceProposal, principle: any): {
    compliant: boolean;
  ...
  async getEthicsVersions(): Promise<Record<string, string>> {
