...
class DAHAOServer {
    ...
    async createProposal(title, description, type, domain, author) {
        try {
            const proposal = {
                id: `DIP-${Date.now()}`,
                title,
    ...
    async validateEthicsCompatibility(proposal) {
        try {
            return await this.ethicsValidator.validateProposal(proposal);
        }
        catch (error) {
    ...
    async analyzeCrossDomainImpact(proposal) {
        try {
            return await this.crossDomainAnalyzer.analyzeCrossDomainImpact(proposal);
        }
        catch (error) {
