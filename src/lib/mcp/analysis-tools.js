...
async function resolveInheritedElements(branchId, elementType) {
    ...
    if (elementType) {
        ...
        allElements.push(...relevantElements.map((el) => ({ ...el, type: elementType })));
    }
    else {
        // Get all types
        for (const [type, elements] of Object.entries(elementSources)) {
            ...
            allElements.push(...relevantElements.map((el) => ({ ...el, type })));
        }
    }
    return allElements;
}
// Helper function to analyze element usage patterns
async function analyzeElementUsage(elementId, branchId) {
    const { elementsTerms, elementsPrinciples, elementsRules, elementsMetarules } = await loadGovernanceData();
    const allElements = [
    ...
    allElements.forEach(element => {
        ...
        const currentVersionData = element.versions?.[element.currentVersion] || {};
        // Check in definition
        ...
        if (currentVersionData.keyRequirements) {
            currentVersionData.keyRequirements.forEach((req) => {
...
async function extractBranchPhilosophy(branchId) {
    const branch = await loadBranchData(branchId);
...
async function getElementUsageCount(elementId, branchId) {
...
export const getBranchElementsTool = tool({
    ...
    execute: async ({ branchId, elementType }) => {
        try {
            ...
            const elementSummaries = await Promise.all(elements.map(async (el) => {
    ...
    execute: async ({ elementId, branchId, version }) => {
        try {
            ...
            if (version && element.currentVersion !== version) {
                ...
                if (element.versions && element.versions[version]) {
